...
const getSubscribeFormSchema = (locale: 'en' | 'ar' = 'ar') => {
	const messages = {
		ar: {
...
type SubscribeFormData = z.infer<ReturnType<typeof getSubscribeFormSchema>>
...
export default function Subscribe({
	...
	const translations = {
		ar: {
			...
			buttonText: buttonText || 'اشترك',
			successMessage:
				successMessage || 'سيتم تواصل معاك لتصميم تجربتك المجانية',
		...
		en: {
			...
			successMessage:
				successMessage || 'You will be contacted to design your free trial',
	...
	const formSchema = getSubscribeFormSchema(locale)
	...
	const onSubmit = async (
		...
		try {
			...
			alert(
				locale === 'ar'
					? `حدث خطأ أثناء الاشتراك: ${errorMessage}`
					: `An error occurred while subscribing: ${errorMessage}`,
			)
		} finally {
				...
				{isSubmitted ? (
					...
					>
						...
						>
							...
							<div className="relative z-10 flex flex-col gap-4 sm:gap-5 md:gap-6 lg:gap-7">
								...
								>
									...
									>
										...
										<m.div
											className="absolute inset-0 bg-gradient-to-r from-teal-600 via-teal-500 to-cyan-600"
											animate={{
							...
							{/* Shine effect on hover */}
							<div className="pointer-events-none absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-white/20 to-transparent opacity-0 transition-all duration-1000 group-hover:translate-x-full group-hover:opacity-100" />
						</form>
					</m.div>
				)}
			</div>
